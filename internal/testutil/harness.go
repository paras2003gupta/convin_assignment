...
// NewServerFull starts an in-process HTTP server backed by the configured
// Postgres and Redis, and returns it alongside the store and service.
// The service is exposed so tests can call Shutdown() to drain in-flight
// background goroutines and then assert on their side-effects.
func NewServerFull(t *testing.T) (*httptest.Server, *store.Store, *ingest.Service) {
	t.Helper()
	cfg := config.Load()
	...
	// Drain any lingering goroutines at test teardown so the next test starts
	// with a clean slate and the DB is not updated after cleanup runs.
